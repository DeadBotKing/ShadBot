...
class AgentPlatformBootstrap:
    ...
    def build(self) -> AgentExecutionLoop:
        ...
        if not workspace_root.exists():
            workspace_root = (Path.cwd() / "ShadBotWorkspace").resolve()

        for project_path in (
            workspace_root / "Meryx",
            workspace_root / "Trader",
            workspace_root / "ShadBotCore_BuiltByAgent",
        ):
            project_path.mkdir(
                parents=True,
                exist_ok=True,
        ...
        projects_list = [
            ...
            Project(
                name="ShadBotCore_BuiltByAgent",
                path=workspace_root / "ShadBotCore_BuiltByAgent",
                project_type="software",
            ),
        ]

        known_names = {p.name for p in projects_list}
